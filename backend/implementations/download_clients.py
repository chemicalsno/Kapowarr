...
class UsenetDownload(ExternalDownload, BaseDirectDownload):
    """Usenet download via NZBHydra2 and Sabnzbd."""
    ...
    def __init__(
        ...
        Args:
            download_link (str): NZB URL from NZBHydra2.
            volume_id (int): The volume ID.
            covered_issues (Union[float, Tuple[float, float], None]): Issue(s) covered.
            source_type (DownloadSource): Should be DownloadSource.USENET.
            source_name (str): Name of the indexer (e.g., "NZBHydra2").
            web_link (Union[str, None]): Link to the webpage.
            web_title (Union[str, None]): Title of the webpage/release.
            web_sub_title (Union[str, None]): Subtitle of the webpage.
            forced_match (bool, optional): Force matching even if issue not found.
                Defaults to False.
        ...
        # Generate filename body for renaming
        self._filename_body = ''
        if settings.rename_downloaded_files:
            try:
                self._filename_body = generate_issue_name(
        ...
        # For Usenet, we don't know the final filename until Sabnzbd completes
        # So use web_title or a generic name as title
        if not self._filename_body:
            self._filename_body = web_title or 'usenet_download'
    ...
    def run(self) -> None:
        ...
        Raises:
            ...
            ClientNotWorking: If cannot connect to Sabnzbd.
        """
    ...
    def update_status(self) -> None:
        ...
        When download completes, extracts the storage_path from Sabnzbd's
        history and maps it to the local filesystem path.

        Also detects encrypted downloads and marks them as failed with
        ...
        # Check for encrypted download
        if usenet_status.get('is_encrypted', False):
            LOGGER.error(
                f"Usenet download {self.external_id} is encrypted "
                "and cannot be processed"
            )
            self._state = DownloadState.FAILED_STATE
            # Note: We don't raise EncryptedDownloadException here because
            # update_status is called in a loop. The failed state will be
            # handled by the download queue manager.
            return
        ...
        # CRITICAL: When download completes, get the storage path
        if self._state == DownloadState.IMPORTING_STATE and 'storage_path' in usenet_status:
            storage_path = usenet_status['storage_path']

            # Map remote path to local path if needed
            ...
            LOGGER.debug(f"Usenet download completed: {local_path}")

        return

    def remove_from_client(self, delete_files: bool) -> None:
        """Remove the download from Sabnzbd.
    ...
    def stop(
        ...
        Args:
            state (DownloadState, optional): State to set.
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Get dict representation of the download.
