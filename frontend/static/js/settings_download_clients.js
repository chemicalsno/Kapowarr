...
function createApiTokenInput(id) {
	...
	input_wrapper.className = 'password-input-wrapper';
	
	const token_input = document.createElement('input');
	token_input.type = 'password';
	token_input.id = id;
	input_wrapper.appendChild(token_input);
	
	const toggle_btn = document.createElement('button');
	toggle_btn.type = 'button';
	toggle_btn.className = 'password-toggle';
	toggle_btn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor"><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/></svg>';
	toggle_btn.onclick = () => {
		const isPassword = token_input.type === 'password';
		token_input.type = isPassword ? 'text' : 'password';
		toggle_btn.classList.toggle('showing', isPassword);
	};
	input_wrapper.appendChild(toggle_btn);
	
	token_container.appendChild(input_wrapper);
...
function loadEditTorrent(api_key, id) {
	...
	.then(client_data => {
		...
		.then(options => {
			const client_options = options.result[client_type].required_tokens;
...
const DOWNLOAD_TYPE_TORRENT = 2;
const DOWNLOAD_TYPE_USENET = 3;
...
function loadTorrentList(api_key) {
	...
	fetchAPI('/externalclients/options', api_key)
	.then(json => {
		Object.entries(json.result).forEach(([name, options]) => {
			// Only show torrent clients
			if (options.download_type !== DOWNLOAD_TYPE_TORRENT) return;
			
			const entry = document.createElement('button');
			entry.innerText = name;
			entry.onclick = e => loadAddTorrent(api_key, name);
...
function loadUsenetList(api_key) {
	...
	.then(json => {
		Object.entries(json.result).forEach(([name, options]) => {
			// Only show usenet clients
			if (options.download_type !== DOWNLOAD_TYPE_USENET) return;
			
			const entry = document.createElement('button');
			entry.innerText = name;
			entry.onclick = e => loadAddUsenet(api_key, name);
			table.appendChild(entry);
		});
...
function loadAddTorrent(api_key, client_type) {
	...
	fetchAPI('/externalclients/options', api_key)
	.then(json => {
		const client_options = json.result[client_type].required_tokens;
...
function loadTorrentClients(api_key) {
	...
	.then(json => {
		...
		json.result.forEach(client => {
			// Only show torrent clients (download_type: 2)
			if (client.download_type !== DOWNLOAD_TYPE_TORRENT) return;
		...
		// Also add usenet clients to remote mapping dropdowns
		json.result.forEach(client => {
			if (client.download_type !== DOWNLOAD_TYPE_USENET) return;
...
function loadUsenetClients(api_key) {
	...
	.then(json => {
		...
		json.result.forEach(client => {
			// Only show usenet clients (download_type: 3)
			if (client.download_type !== DOWNLOAD_TYPE_USENET) return;
